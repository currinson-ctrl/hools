 ...
 */
export async function searchRelatedImage(query: string): Promise<string | null> {
  try {
    const url = `https://api.openverse.org/v1/images/?q=${encodeURIComponent(
      query
    )}&page_size=3&license_type=commercial&mature=false`;
    ...
    const match = json.results?.find((r) => typeof r.url === "string" && r.url.length > 0);
